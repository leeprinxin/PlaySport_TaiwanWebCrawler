...
class NFLWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.NFL_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={93}&gametime={day}" for day in days]
        ...
        self.SportCode = 7  # NFL運動編碼: 2 來自SportCode table
        self.SportTournamentCode = '10037219' # 沿用
        self.EventType = 0
        self.CollectClient = 'NFL'
        ...
        self.Games = '61' # 運動聯盟編號
        self.SportText = 'American football'
        self.TournamentText = 'NFL'
    ...
    def get_MatchResultsBySelenium(self, day='1天'):
        """
        從https://www.sportslottery.com.tw/zh-tw/game/pre取得NFL今日結束比賽
        :param
        :return:
        """
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'美式足球')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'美國')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'國家美式足球聯盟')]").click()
        sleep(1)
        ...
        try:
            ...
            while True:
                ...
                for table, mydate in zip(
                        objSoup.select('div.tslc-search-2 div.tslc-search-result div.table-responsive')[:],
                        objSoup.select("div.tslc-search-2 div.tslc-search-result div.tslc-date")[:]):
                    ...
                    for tbody in tbodys:
                        ...
                        for i in range(0, len(games), 2):
                            ...
                            HomeTeam = self.TeamNameCorrection(tds_h[1].text, cursor, 1)
                            AwayTeam = self.TeamNameCorrection(tds_a[4].text, cursor, 1)

                            MatchTime = datetime.strptime(mydate + ' ' + tds_a[0].text + ':00',
                                                          "%Y/%m/%d %H:%M:%S").strftime("%Y-%m-%d %H:%M:%S.000")
                            ...
                            EventCode_Prefix = self.Games + '_' + MatchTime.strftime(
                                "%Y%m%d") + '_'  # 聯盟編號_比賽日期 ex. 2_20211213
                            ...
                            MatchResult = dict(EventCode=EventCode, TournamentText=self.TournamentText,
                                               MatchTime=MatchTime
    ...
    def get_Odds(self):
        ...
        QueryGroupOptionCode = {'td-bank-bet03': 20, 'td-bank-bet01': 51, 'td-bank-bet02': 52}
        QueryOptionCode = {'20,home': 1, '20,away': 2, '51,home': 1, '51,away': 2, '52,Over': 'Over',
                           '52,Under': 'Under'}
        ...
        for game_dict in self.MatchEntry:
            ...
            for away_odds_html, home_odds_html in zip(Odds_html[0], Odds_html[1]):
                try:
                    ...
                    CreatedTime = datetime.now().astimezone(timezone(timedelta(hours=8))).strftime(
                        "%Y-%m-%d %H:%M:%S.000")
                    play_dict_H = dict(GroupOptionCode=GroupOptionCode, OptionCode=OptionCode_H,
                                       SpecialBetValue=SpecialBetValue_H, OptionRate=OptionRate_H,
                                       SourceCode=SourceCode, SportCode=SportCode,
                                       SportTournamentCode=SportTournamentCode, EventCode=EventCode,
                    ...
                    play_dict_A = dict(GroupOptionCode=GroupOptionCode, OptionCode=OptionCode_A,
                                       ...
                                       CreatedTime=CreatedTime)
                    try:
                        self.Odds[game_dict['EventCode']] = self.Odds[game_dict['EventCode']] + [play_dict_H,
                                                                                                 play_dict_A]
                    except:
    ...
    def get_MatchEntry(self):
        """
        從https://www.playsport.cc/predictgame.php?allianceid=3&from=header取得NFL所有比賽
        :param
        :return:
        """
        ...
        for url in self.NFL_UpcomingMacths_URLs:
            response = scraper.get(url).text
            time.sleep(3)
            objSoup = bs4.BeautifulSoup(response, 'lxml')
            games = objSoup.select('table.predictgame-table tr[gameid]')
            for i in range(0, len(games), 2):
                # 取得比賽時間
                ...
                if not games[i].select('td.td-teaminfo')[0].has_attr('rowspan') and datetime.now().astimezone(timezone(timedelta(hours=8))).replace(tzinfo=utc) <= MatchTime.replace(tzinfo=utc):  # 檢查是否完賽或開賽中
                    # 取得隊伍
                    AwayTeam = self.TeamNameCorrection(games[i].select('td.td-teaminfo')[0].getText().strip(), cursor, byChinese=1)
                    HomeTeam = self.TeamNameCorrection(games[i + 1].select('td.td-teaminfo')[0].getText().strip(), cursor, byChinese=1)

                    if self.get_Alliance(HomeTeam, cursor) == self.TournamentText and self.get_Alliance(AwayTeam, cursor) == self.TournamentText:  # 檢查是否為指定聯盟
                        # 轉換Bet365時間
...
if __name__ == '__main__':
    ...
    NFLWebCrawler.start(MatchResultsSearchDays='1週')