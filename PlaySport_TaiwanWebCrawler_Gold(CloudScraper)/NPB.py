...
class NPBWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.NPB_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={2}&gametime={day}" for day in days]
        ...
        self.SportCode = 3 # 棒球運動編碼: 3 來自SportCode table
        self.SportTournamentCode = '10041830' # 沿用
        self.EventType = 0
        self.CollectClient = 'NPB'
        ...
        self.Games = '22' # 運動聯盟編號
        self.SportText = 'Baseball'
        self.TournamentText = 'NPB'
    ...
    def start(self, MatchResultsSearchDays = '1天'):
        ...
        self.MatchResults = []
        for k in ['中央聯盟','太平洋聯盟']:
            self.get_MatchResultsBySelenium(day = MatchResultsSearchDays, keyword=k)
    ...
    def get_MatchResultsBySelenium(self, keyword , day = '1天'):
        """
        從https://www.sportslottery.com.tw/zh-tw/game/pre取得CBPL今日結束比賽
        :param
        :return:
        """
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'日本')]").click()
        sleep(1)
        browser.find_element_by_xpath(f"//option[contains(text(),'{keyword}')]").click()
        sleep(1)
    ...
    def add_MatchEntry_or_Odds(self):
        ...
        for game_dict in self.MatchEntry:
            # 更改Bet365投手
            self.set_Bet365Pitcher(game_dict['MatchTime'], 60, game_dict['HomeTeam'],
                                   game_dict['AwayTeam'], game_dict['HomePitcher'], game_dict['AwayPitcher'],cursor,db)
    ...
    def set_Bet365Pitcher(self,mydatatime, offset_minute, HomeTeam, AwayTeam, HomePitcher, AwayPitcher, cursor, db):
        ...
        sql = f"SELECT EventCode FROM MatchEntry where SourceCode = 'Bet365' AND MatchTime > '{bottom}' AND MatchTime < '{top}' AND HomeTeam = '{HomeTeam}' AND AwayTeam = '{AwayTeam}' "
        cursor.execute(sql)
        result = cursor.fetchone()
        if result:
            update_sql = f'''UPDATE [dbo].[MatchEntry] SET [HomePitcher]=N'{HomePitcher}',[AwayPitcher]=N'{AwayPitcher}' WHERE [EventCode] = '{result[0]}' '''
            cursor.execute(update_sql)
            db.commit()
    ...
    def get_MatchEntry(self):
        ...
        for url in self.NPB_UpcomingMacths_URLs:
...
if __name__ == '__main__' :     
  NPBWebCrawler = NPBWebCrawler()
  print('MatchResultsSearchDays = [1天, 3天, 1週, 3週, 1月, 2月, 3月]')
  NPBWebCrawler.start(MatchResultsSearchDays='3天')