...
class BundesligaWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.Bundesliga_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={4}&gametime={day}" for day in days]
        ...
        self.SportCode = 1  # Bundesliga運動編碼: 1 來自SportCode table
        self.SportTournamentCode = '10041095'  # 沿用
        self.EventType = 0
        ...
        self.TournamentText = 'Bundesliga'
    ...
    def get_MatchResultsBySelenium(self, day = '1天'):
        """
        從https://www.sportslottery.com.tw/zh-tw/game/pre取得Bundesliga今日結束比賽
        :param
        :return:
        """
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'德國')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'德國甲級聯賽')]").click()
        sleep(1)
    ...
    def get_MatchEntry(self):
        """
        從https://www.playsport.cc/predictgame.php?allianceid=4&from=header取得Bundesliga所有比賽
        :param
        :return:
        """
        ...
        for url in self.Bundesliga_UpcomingMacths_URLs:
...
if __name__ == '__main__':
    ...
    BundesligaWebCrawler.start(MatchResultsSearchDays='3週')