...
class LaLigaWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.LaLiga_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={4}&gametime={day}" for day in days]
        ...
        self.SportCode = 1  # Ligue1運動編碼: 1 來自SportCode table
        self.SportTournamentCode = '10041100'  # 沿用
        self.EventType = 0
    ...
    def get_MatchResultsBySelenium(self, day = '1天'):
        """
        從https://www.sportslottery.com.tw/zh-tw/game/pre取得LaLiga今日結束比賽
        :param
        :return:
        """
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'足球')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'西班牙')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'西班牙甲級聯賽')]").click()
        sleep(1)
    ...
    def get_Odds(self):
        ...
        QueryGroupOptionCode = {'td-bank-bet03': 10, 'td-bank-bet02': 52}
        QueryOptionCode = {'10,home': 1, '10,away': 2, '10,Draw': 'X', '52,Over': 'Over', '52,Under': 'Under'}

        factor = 0 # 賠率因子
        for game_dict in self.MatchEntry:
            Odds_html = game_dict['Odds_html']
            for away_odds_html,home_odds_html,odds_html  in zip(Odds_html[0],Odds_html[1],Odds_html[2]):
                try:
                    ...
                    if GroupOptionCode == 10:
                        ...
                        OptionRate_T = odds_html.div.label.span.span.getText()
                    ...
                    if not GroupOptionCode == 52:
                        play_dict_T = dict(GroupOptionCode=GroupOptionCode, OptionCode=OptionCode_T,
                                           SpecialBetValue=SpecialBetValue_T, OptionRate=OptionRate_T,
                                           SourceCode=SourceCode,
                                           SportCode=SportCode, SportTournamentCode=SportTournamentCode,
                                           EventCode=EventCode,
                                           EventType=EventType, CollectClient=CollectClient,
    ...
    def get_MatchEntry(self):
        """
        從https://www.playsport.cc/predictgame.php?allianceid=4&from=header取得LaLiga所有比賽
        :param
        :return:
        """
        ...
        for url in self.LaLiga_UpcomingMacths_URLs:
            ...
            for i in range(0,len(games),3):
                ...
                if not games[i].select('td.td-teaminfo')[0].has_attr('rowspan') and datetime.now().astimezone(
                        timezone(timedelta(hours=8))).replace(tzinfo=utc) <= MatchTime.replace(
                        tzinfo=utc):  # 檢查是否完賽或開賽中
                    # 取得隊伍
                    ...
                    if self.get_Alliance(HomeTeam, cursor) == self.TournamentText and self.get_Alliance(AwayTeam, cursor) == self.TournamentText:  # 檢查是否為指定聯盟
                        ...
                        # 取得賠率
                        odds_html = [games[i].select("td[class^='td-bank-bet']"),games[i + 1].select("td[class^='td-bank-bet']"),games[i + 2].select("td[class^='td-bank-bet']")]
                        # 整理
...
if __name__ == '__main__':
    LaLigaWebCrawler = LaLigaWebCrawler()
    print('MatchResultsSearchDays = [1天, 3天, 1週, 3週, 1月, 2月, 3月]')
    LaLigaWebCrawler.start(MatchResultsSearchDays='3天')