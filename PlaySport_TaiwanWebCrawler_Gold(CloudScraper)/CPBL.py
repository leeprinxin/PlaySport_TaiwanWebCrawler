...
class CPBLWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.CPBL_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={6}&gametime={day}" for day in days]
        ...
        self.Games = '23' # 運動聯盟編號
        self.SportText = 'Baseball'
        self.TournamentText = 'CPBL'
    ...
    def get_MatchResultsBySelenium(self, day = '1天'):
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'臺灣')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'中華職棒')]").click()
        sleep(1)
    ...
    def get_MatchEntry(self):
        ...
        for url in self.CPBL_UpcomingMacths_URLs:
...
if __name__ == '__main__' :     
  CPBLWebCrawler = CPBLWebCrawler()
  print('MatchResultsSearchDays = [1天, 3天, 1週, 3週, 1月, 2月, 3月]')
  CPBLWebCrawler.start(MatchResultsSearchDays='3天')