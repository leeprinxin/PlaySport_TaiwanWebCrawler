...
class Ligue1WebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.Ligue1_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={4}&gametime={day}" for day in days]
    ...
    def get_MatchResultsBySelenium(self, day = '1天'):
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'法國')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'法國甲級聯賽')]").click()
        sleep(1)
    ...
    def get_MatchEntry(self):
        """
        從https://www.playsport.cc/predictgame.php?allianceid=4&from=header取得Ligue1所有比賽
        :param
        :return:
        """
        ...
        for url in self.Ligue1_UpcomingMacths_URLs:
...
if __name__ == '__main__':
    Ligue1WebCrawler = Ligue1WebCrawler()
    print('MatchResultsSearchDays = [1天, 3天, 1週, 3週, 1月, 2月, 3月]')
    Ligue1WebCrawler.start(MatchResultsSearchDays='3天')