...
class NBAWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.NBA_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={3}&gametime={day}" for day in days]
        ...
        self.SportCode = 2 # 籃球運動編碼: 2 來自SportCode table
        ...
        self.Games = '11' # 運動聯盟編號
        self.SportText = 'Basketball'
    ...
    def get_MatchResultsBySelenium(self, day = '1天'):
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'籃球')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'美國')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'美國職籃季後賽')]").click()
        sleep(1)
    ...
    def get_Odds(self):
        ...
        QueryOptionCode = {'20,home':1,'20,away':2,'51,home':1,'51,away':2,'52,Over':'Over','52,Under':'Under'}
    ...
    def get_MatchEntry(self):
        ...
        for url in self.NBA_UpcomingMacths_URLs:
...
if __name__ == '__main__' :     
  ...
  NBAWebCrawler.start(MatchResultsSearchDays='3天')