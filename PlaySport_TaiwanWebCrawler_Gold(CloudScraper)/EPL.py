...
class EPLWebCrawler(object):
    def __init__(self):
        # 即將舉行的比賽
        days = [datetime.now().strftime('%Y%m%d'), (datetime.now()+timedelta(days=1)).strftime('%Y%m%d')]
        self.EPL_UpcomingMacths_URLs = [f"https://www.playsport.cc/predictgame.php?allianceid={4}&gametime={day}" for day in days]
        ...
        self.SportCode = 1  # EPL運動編碼: 1 來自SportCode table
        self.SportTournamentCode = '10041282'  # 沿用
        self.EventType = 0
    ...
    def get_MatchResultsBySelenium(self, day = '1天'):
        ...
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'英格蘭')]").click()
        sleep(1)
        browser.find_element_by_xpath("//option[contains(text(),'英格蘭超級聯賽')]").click()
        sleep(1)
    ...
    def get_MatchEntry(self):
        """
        從https://www.playsport.cc/predictgame.php?allianceid=4&from=header取得EPL所有比賽
        :param
        :return:
        """
        ...
        for url in self.EPL_UpcomingMacths_URLs:
...
if __name__ == '__main__':
    EPLWebCrawler = EPLWebCrawler()
    print('MatchResultsSearchDays = [1天, 3天, 1週, 3週, 1月, 2月, 3月]')
    EPLWebCrawler.start(MatchResultsSearchDays='3天')